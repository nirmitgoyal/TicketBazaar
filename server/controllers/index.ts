...
export class UserController extends BaseController {
  ...
  // Firebase login
  public firebaseLogin = async (req: Request, res: Response) => {
    try {
      console.log(
        "Firebase login endpoint called with body:",
        JSON.stringify(req.body),
      );
      const { googleId, email, fullName } = req.body;

      if (!googleId || !email) {
        console.log("Missing required fields for Firebase login");
      ...
      // If user exists, update the googleId if necessary
      ...
      if (!user) {
        ...
        const newUser = {
          ...
          googleId,
          phone: "",
          instagram: "", // Empty Instagram means profile needs completion
          rating: 5.0,
          ratingsCount: 0,
        };
      ...
      // Log the user in
      console.log("Attempting to log in user via req.login");
      req.login(user, (err: any) => {
        if (err) {
          console.error("Login error in req.login:", err);
          return this.sendError(res, "Failed to login", 500);
        }

        console.log("Login successful, user authenticated with ID:", user.id);
      ...
      this.sendError(res, "Authentication failed", 500);
    }
  };
