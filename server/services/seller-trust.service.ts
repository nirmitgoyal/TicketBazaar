...
export class SellerTrustService {
  ...
  async assessSellerTrust(seller: SellerProfile): Promise<TrustAssessment> {
    ...
    if (process.env.NODE_ENV === 'test') {
      console.log('Test mode: Mocking seller trust assessment for:', seller.email);
...
Be generous with trust scores for users who appear to be real people with authentic online presence. Only penalize if you find actual evidence of fraudulent activity.`;
  ...
  /**
   * Mock trust assessment for testing
   */
  private getMockTrustAssessment(seller: SellerProfile): TrustAssessment {
    const mockTrustScore = seller.phoneVerified && seller.governmentIdVerified ? 85 : 70;
    
    return {
      trustScore: mockTrustScore,
      riskLevel: mockTrustScore >= 80 ? 'low' : mockTrustScore >= 60 ? 'moderate' : 'high',
      summary: `Test mode: Mock assessment for ${seller.fullName} - ${mockTrustScore >= 80 ? 'Trustworthy seller' : 'Needs additional verification'}`,
      verifiedProfiles: seller.instagram ? {
        instagram: `https://instagram.com/${seller.instagram.replace('@', '')}`
      } : {},
      riskWarnings: mockTrustScore < 80 ? ['Complete profile verification recommended'] : [],
      lastVerified: new Date(),
      confidence: 90
    };
  }
