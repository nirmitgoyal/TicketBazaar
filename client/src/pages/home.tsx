...
export default function Home() {
  ...
  const [displayedTicketsCount, setDisplayedTicketsCount] =
    useState<number>(TICKETS_PER_PAGE);
  ...
  const isFutureTicket = (ticket: Ticket): boolean => {
    try {
      ...
      const eventInVenueTimezoneString = new Date(
        eventDateTimeMs,
      ...
      const nowInVenueMs = new Date(
        nowInVenueTimezoneString.replace(", ", "T"),
      ).getTime();
      const eventInVenueMs = new Date(
        eventInVenueTimezoneString.replace(", ", "T"),
      ).getTime();
  ...
  return (
    <>
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto text-center space-y-8">
            <header className="space-y-4">
              ...
              >
                Resell or Buy tickets for comedy shows, movies, bus, concerts,
                sports, etc
              </p>
            </header>
            ...
            {/* Search Bar */}
            <div
              className="max-w-2xl mx-auto bg-white rounded-lg p-2 space-y-2 sm:space-y-0 sm:flex sm:items-center sm:space-x-2"
              role="search"
      ...
      <section
        ...
        role="main"
        aria-label="Available second-hand tickets"
      >
        <div className="container mx-auto px-4">
          <div className="flex items-center justify-between mb-6">
              ...
              {searchQuery.length >= 2
                ? searchResults.length > 0 && (
                    <p className="text-sm text-gray-600 mt-1">
                      Showing{" "}
                      {Math.min(
                        searchResults.filter(isFutureTicket).length,
                        TICKETS_PER_PAGE,
                      )}{" "}
                      ticket
                      {Math.min(
            ...
            <Button
              ...
              aria-label="Filter ticket results"
            >
              <span>Filter</span>
              <svg
                className="w-4 h-4"
          ...
          {searchQuery.length >= 2 ? (
            ...
            searchLoading ? (
              ...
              <div className="mobile-grid gap-3 sm:gap-4 lg:gap-6">
                {searchResults
                  .filter(isFutureTicket)
                  .slice(0, TICKETS_PER_PAGE)
                  .map((ticket) => (
                    <div
                      key={ticket.id}
          ...
          ticketsLoading ? (
            ...
            <div className="mobile-grid gap-3 sm:gap-4 lg:gap-6">
              {(defaultTickets.length > 0 ? defaultTickets : initialTickets)
                .filter(isFutureTicket)
            ...
            !defaultTickets.length && (
              <div className="mobile-grid gap-3 sm:gap-4 lg:gap-6">
                ...
                {(() => {
                  ...
                  const sampleTickets = Array.from(
                  ...
                  const futureSampleTickets =
                    sampleTickets.filter(isFutureTicket);
                  const sortedSampleTickets = futureSampleTickets.sort(
                    (a, b) => {
              ...
              <div className="text-center mt-8">
                <Button
                  ...
                  disabled={
                    !(
                      hasMoreTickets ||
                      defaultTickets.filter(isFutureTicket).length >
                        displayedTicketsCount
                    ) || isLoadingMore
                  }
                >
